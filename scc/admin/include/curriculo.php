...
<?
		...
		$inicial = ($pg_num * $reg_num);
			...
			$regs = mysql_query("SELECT * FROM ".$_GET["navega"]." ORDER BY nome LIMIT $inicial,$reg_num");		
			$contador01 = "SELECT * FROM ".$_GET["navega"]." ORDER BY nome";					
...
?>
          ...
          <td width="50%" class="td-cinza04">
                            ...
                            <select name="registros" size="1" class="input-A" id="registros" onchange="if(options[selectedIndex].value) document.location = ('?reg_num='+options[selectedIndex].value+'&navega=<? echo $_GET["navega"]; ?>');">
	...
	if($pg_num <> 0){
		$url = $pg_num - 1;
		print "<a href=\"$PHP_SELF?reg_num=".$reg_num."&pg_num=".$url."&navega=".$_GET["navega"]."\" class=\"link-menu-cinza\"><</a>|&nbsp;";
	}else{
		print "<|&nbsp;";
	}
	$a="20";
	for ($i = 0; $i<($paginas + 1); $i++) {
		if ($i == $pg_num){
			...
			print "<a href=\"$PHP_SELF?reg_num=".$reg_num."&pg_num=".$i."&navega=".$_GET["navega"]."\" class=\"link-menu-cinza\">$i</a>&nbsp;";		
